...
st.markdown("تحليل بيانات الشبكة ورسم خرائط حرارية تنبؤية مع التنبيهات والتحليل الزمني والتقارير")
...
grid_df["pred_rssi"] = grid_df["pred_score"] * (-50 + 120) - 120

# ------------------------- تنبيهات المناطق الضعيفة -------------------------
weak_zones = grid_df[grid_df["pred_score"] < 0.3]

# ------------------------- Tabs للعرض -------------------------
tab1, tab2, tab3 = st.tabs(["🗺️ الخريطة", "📈 التحليل الزمني", "📊 التقارير"])
...
with tab1:
    ...
    if not use_real_data:
        for _, t in towers.iterrows():
            folium.CircleMarker([t.lat, t.lon], radius=6, color="blue", fill=True).add_to(m)

    for _, row in weak_zones.iterrows():
        folium.CircleMarker([row.lat, row.lon], radius=4, color="red", fill=True).add_to(m)

    st_folium(m, width=1100, height=600)
    st.write("✅ الخريطة تم إنشاؤها بنجاح")

# ---- التحليل الزمني ----
with tab2:
    st.subheader("📊 متوسط قوة الإشارة على مدار اليوم")
    hourly_avg = data.groupby("hour")["score"].mean().reset_index()
    st.line_chart(hourly_avg, x="hour", y="score")
    st.caption("كل نقطة تمثل متوسط جودة الإشارة لكل ساعة")

# ---- التقارير ----
with tab3:
    st.subheader("📂 تصدير تقرير التغطية")
    csv = grid_df.to_csv(index=False).encode("utf-8")
    st.download_button("💾 تحميل التقرير (CSV)", csv, "network_report.csv", "text/csv")

    if not weak_zones.empty:
        st.warning("⚠️ مناطق ضعيفة تم اكتشافها:")
        st.dataframe(weak_zones[["lat", "lon", "pred_rssi"]].head(20))
    else:
        st.success("✅ لا توجد مناطق ضعيفة في هذا التوقيت")

st.success("🎉 الخريطة التنبؤية جاهزة بالكامل!")
# network_heatmap_app.py
"""
واجهة تفاعلية (Streamlit) لعرض وتحليل الإشارة وعمل خرائط حرارية تنبؤية
بها: 
